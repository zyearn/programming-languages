  ...
  (cond [(var? e) 
        ...
        [(call? e)
         (let ([c (eval-under-env (call-funexp e) env)]
           ...
           (if (closure? c)
               (let ([f (closure-fun c)]
                     [nenv (closure-env c)])
                 (if (eq? (fun-nameopt f) #f)
                     (eval-under-env (fun-body f) (cons (cons (fun-formal f) v) nenv))
                     (eval-under-env (fun-body f) (cons (cons (fun-nameopt f) c) (cons (cons (fun-formal f) v) nenv)))))
               (error "MUPL call need a closure")))]
        [(apair? e)
        ...
        [(fst? e)
         (let ([r (eval-under-env (fst-e e) env)])
           (if (apair? r)
               (apair-e1 r)
               (error "MUPL fst needs an apair")))]
        [(snd? e)
         (let ([r (eval-under-env (snd-e e) env)])
           (if (apair? r)
               (apair-e2 r)
               (error "MUPL snd needs an apair")))]
        [(aunit? e) e]
        [(isaunit? e)
  ...
  (fun "map-fn" "fn"
       (fun "map-lst" "lst"
            (ifeq (isaunit (var "lst")) (int 1)
                  (aunit)
                  (apair (call (var "fn") (fst (var "lst"))) (call (var "map-lst") (snd (var "lst"))))))))


(define mupl-mapAddN 
  (mlet "map" mupl-map
        (fun "mupl-mapAddN-i" "i"
             (fun "map-add" "lst"
